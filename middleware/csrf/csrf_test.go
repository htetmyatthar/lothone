...
func TestValidToken(t *testing.T) {
	...
	if !validTokenAtTime(tok, key, userID, actionID, oneMinuteFromNow, csrfTimeout) {
		t.Error("One second later: Expected token to be valid")
	}
	if !validTokenAtTime(tok, key, userID, actionID, now.Add(csrfTimeout-1*time.Nanosecond), csrfTimeout) {
		t.Error("Just before timeout: Expected token to be valid")
	}
	if !validTokenAtTime(tok, key, userID, actionID, now.Add(-1*time.Minute+1*time.Millisecond), csrfTimeout) {
...
func TestInvalidToken(t *testing.T) {
	invalidTokenTests := []struct {
		...
		{"Bad key", "foobar", userID, actionID, oneMinuteFromNow, csrfTimeout},
		{"Bad userID", key, "foobar", actionID, oneMinuteFromNow, csrfTimeout},
		{"Bad actionID", key, userID, "foobar", oneMinuteFromNow, csrfTimeout},
		{"Expired", key, userID, actionID, now.Add(csrfTimeout + 1*time.Millisecond), csrfTimeout},
		{"More than 1 minute from the future", key, userID, actionID, now.Add(-1*time.Nanosecond - 1*time.Minute), csrfTimeout},
...
func TestValidateBadData(t *testing.T) {
	...
	for _, bdt := range badDataTests {
		if validTokenAtTime(bdt.tok, key, userID, actionID, oneMinuteFromNow, csrfTimeout) {
